...
const Checkout = () => {
    ...
    const [formData, setFormData] = useState({ company: '', name: '', role: '', email: '' });
    ...
    const handleCheckout = (e) => {
        ...
        setTimeout(() => {
            alert(`Initialization complete for ${formData.name} at ${formData.company}. Welcome to KAI.`);
                ...
                <div className="max-w-xl w-full mx-auto">
                    {step === 1 ? (
                        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                            ...
                            <p className="text-white/50 text-sm mb-8">Please provide your details for the invoice before proceeding to payment.</p>

                            <form onSubmit={handleRegistrationSubmit} className="flex flex-col gap-6">
                                <div className="relative group">
                                    <input type="text" required placeholder="Company Name" className="peer w-full bg-charcoal border border-white/10 rounded-xl px-4 py-4 text-sm font-sans text-white focus:outline-none focus:border-teal/50 focus:shadow-[0_0_15px_rgba(0,161,159,0.2)] transition-all placeholder-transparent" value={formData.company} onChange={(e) => setFormData({ ...formData, company: e.target.value })} />
                                    <label className="absolute left-4 top-4 text-sm text-white/40 transition-all peer-focus:-top-2 peer-focus:text-xs peer-focus:text-teal peer-focus:bg-[#050505] peer-focus:px-1 peer-valid:-top-2 peer-valid:text-xs peer-valid:bg-[#050505] peer-valid:px-1 cursor-text pointer-events-none">Company Name</label>
                                </div>
                                <div className="relative group">
                                    <input type="text" required placeholder="Full Name" className="peer w-full bg-charcoal border border-white/10 rounded-xl px-4 py-4 text-sm font-sans text-white focus:outline-none focus:border-teal/50 focus:shadow-[0_0_15px_rgba(0,161,159,0.2)] transition-all placeholder-transparent" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} />
                                    <label className="absolute left-4 top-4 text-sm text-white/40 transition-all peer-focus:-top-2 peer-focus:text-xs peer-focus:text-teal peer-focus:bg-[#050505] peer-focus:px-1 peer-valid:-top-2 peer-valid:text-xs peer-valid:bg-[#050505] peer-valid:px-1 cursor-text pointer-events-none">Full Name</label>
                                </div>
                                <div className="flex flex-col md:flex-row gap-6">
                                    <div className="relative group w-full md:w-1/2">
                                        <input type="text" required placeholder="Role / Title" className="peer w-full bg-charcoal border border-white/10 rounded-xl px-4 py-4 text-sm font-sans text-white focus:outline-none focus:border-teal/50 focus:shadow-[0_0_15px_rgba(0,161,159,0.2)] transition-all placeholder-transparent" value={formData.role} onChange={(e) => setFormData({ ...formData, role: e.target.value })} />
                                        <label className="absolute left-4 top-4 text-sm text-white/40 transition-all peer-focus:-top-2 peer-focus:text-xs peer-focus:text-teal peer-focus:bg-[#050505] peer-focus:px-1 peer-valid:-top-2 peer-valid:text-xs peer-valid:bg-[#050505] peer-valid:px-1 cursor-text pointer-events-none">Role / Title</label>
                                    </div>
                                    <div className="relative group w-full md:w-1/2">
                                        <input type="email" required placeholder="Email Address" className="peer w-full bg-charcoal border border-white/10 rounded-xl px-4 py-4 text-sm font-sans text-white focus:outline-none focus:border-teal/50 focus:shadow-[0_0_15px_rgba(0,161,159,0.2)] transition-all placeholder-transparent" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} />
                                ...
                                <button
                                    type="submit"
                                    className="mt-6 w-full py-4 rounded-xl font-bold text-sm tracking-wider uppercase transition-all duration-300 relative overflow-hidden group shadow-[0_10px_30px_rgba(0,161,159,0.2)] bg-teal text-black hover:shadow-[0_10px_40px_rgba(0,161,159,0.4)]"
                                >
                                    Continue to Payment
                                </button>
                        ...
                        <div className="animate-in fade-in slide-in-from-right-8 duration-500">
                            <div className="flex items-center gap-4 mb-8">
                                <button onClick={() => setStep(1)} className="text-white/50 hover:text-white transition-colors flex items-center justify-center p-2 rounded-full hover:bg-white/5">
